...
def main():
    ...
    graphs = [read(graph_file) for graph_file in fnmatch.filter(
        [os.path.join(root_path, file) for file in os.listdir(root_path)], '*.txt')]

    lapeigs = [laplacian_eigen_vectors(
        graph, 'area', operations[2]) for graph in graphs]
    size = reduce(lambda x, y: min(x, y), [
                  lapeig.shape[1] for lapeig in lapeigs])
    print [lapeig[:, -size:].flatten() for lapeig in lapeigs]
